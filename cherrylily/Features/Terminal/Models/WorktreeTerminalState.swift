...
final class WorktreeTerminalState {
  ...
  private func createSurface(
    ...
    let view = GhosttySurfaceView(
      ...
      initialInput: initialInput,
      environmentVariables: surfaceEnvironment(tabId: tabId, surfaceID: resolvedSurfaceID.rawValue),
  ...
  private func cleanupSurfaceState(for surfaceID: UUID) {
    ...
    surfaceIdToTabId.removeValue(forKey: surfaceID)
    ...
    let wrappedID = SurfaceID(rawValue: surfaceID)
    if let persistence {
      Task.detached { [persistence] in
        try? await persistence.killSession(for: wrappedID)
      }
      try? persistence.deleteScrollback(for: wrappedID)
    }
  }
