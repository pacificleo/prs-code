...
struct RepositoriesFeature {
        ...
        guard let worktree = state.worktree(for: worktreeID),
          ...
          return .send(
            .presentAlert(
              ...
              message: "CherryLily could not find a pull request for this worktree."
            )
          )
        }
          ...
          guard let url = URL(string: pullRequest.url) else {
            return .send(
              .presentAlert(
                ...
                message: "CherryLily could not open the pull request URL."
              )
            )
          }
          ...
          guard let failingCheckDetailsURL, !failingCheckDetailsURL.isEmpty else {
            return .send(
              .presentAlert(
                ...
                message: "CherryLily could not find a failing check URL."
              )
            )
          }
          return .run { send in
        ...
        case .openFailingCheckDetails:
          guard let failingCheckDetailsURL, let url = URL(string: failingCheckDetailsURL) else {
            return .send(
              .presentAlert(
                title: "Failing check not found",
                message: "CherryLily could not find a failing check with details."
              )
            )
          }
          return .run { @MainActor _ in
            ...
            guard !branchName.isEmpty else {
              await send(
                .presentAlert(
                  ...
                  message: "CherryLily could not determine the pull request branch."
                )
              )
              return
            }
              ...
              guard let run = try await githubCLI.latestRun(worktreeRoot, branchName) else {
                ...
                await send(
                  .presentAlert(
                    ...
                    message: "CherryLily could not find any workflow runs for this branch."
                  )
                )
                return
              }
              guard run.conclusion?.lowercased() == "failure" else {
                ...
                await send(
                  .presentAlert(
                    ...
                    message: "CherryLily could not find a failing workflow run to copy logs from."
                  )
                )
                return
              }
            ...
            do {
              ...
              guard run.conclusion?.lowercased() == "failure" else {
                ...
                await send(
                  .presentAlert(
                    title: "No failing workflow run",
                    message: "CherryLily could not find a failing workflow run to re-run."
                  )
                )
                return
              }
  ...
  private func confirmationAlertForRepositoryRemoval(
    ...
    return AlertState {
      ...
      TextState(
        "This removes the repository from CherryLily. "
...
private func cleanupFailedWorktree(
  ...
  let baseDirectory = CherryLilyPaths.repositoryDirectory(for: repositoryRootURL).standardizedFileURL
  let worktreeURL =
    baseDirectory
...
private nonisolated func worktreeCreateCommand(
  ...
  let baseDir = CherryLilyPaths.repositoryDirectory(for: repositoryRootURL).path(percentEncoded: false)
