...
nonisolated struct CLIInstaller {
  private static let installPath = "/usr/local/bin/cherrylily"

  /// Returns the path to the CLI binary inside the app bundle.
  static var bundledCLIPath: String? {
    Bundle.main.resourceURL?
      .appending(path: "bin/cherrylily", directoryHint: .notDirectory)
  ...
  func install() throws {
    ...
    try runPrivileged(
      ...
      prompt: "CherryLily needs administrator access to install the CLI to /usr/local/bin."
    )
  }
  ...
  func uninstall() throws {
    ...
    try runPrivileged(
      ...
      prompt: "CherryLily needs administrator access to uninstall the CLI from /usr/local/bin."
    )
  }
  ...
  /// makes macOS show the CherryLily icon and name in the authorization dialog.
