...
final class GhosttyRuntime {
  ...
  func setColorScheme(_ scheme: ColorScheme) {
    ...
    if lastColorScheme == ghosttyScheme { return }
  ...
  /// Applies CherryLily-specific config (padding values) that takes precedence over user settings.
  private static func loadBundledOverrides(into config: ghostty_config_t) {
    ...
    let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("cherrylily-defaults.conf")
    do {
  ...
  /// When terminal theme sync is enabled, loads the bundled CherryLily
  ...
  private static func loadBundledTheme(into config: ghostty_config_t, enabled: Bool) {
    ...
    guard
      let lightPath = Bundle.main.path(forResource: "CherryLily Light", ofType: nil),
      let darkPath = Bundle.main.path(forResource: "CherryLily Dark", ofType: nil)
    else {
    ...
    let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("cherrylily-theme.conf")
    do {
