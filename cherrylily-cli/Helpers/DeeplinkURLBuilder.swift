...
/// Builds `cherrylily://` deeplink URLs from structured components.
nonisolated enum DeeplinkURLBuilder {
  ...
  static func tabClose(worktreeID: String, tabID: String) -> String {
    "cherrylily://worktree/\(worktreeID)/tab/\(tabID)/destroy"
  }

  // MARK: - Surface.

  static func surfaceFocus(worktreeID: String, tabID: String, surfaceID: String, input: String?) -> String {
    var url = "cherrylily://worktree/\(worktreeID)/tab/\(tabID)/surface/\(surfaceID)"
  ...
  static func surfaceSplit(
    ...
    var url = "cherrylily://worktree/\(worktreeID)/tab/\(tabID)/surface/\(surfaceID)/split"
    var params: [String] = []
  ...
  static func surfaceClose(worktreeID: String, tabID: String, surfaceID: String) -> String {
    "cherrylily://worktree/\(worktreeID)/tab/\(tabID)/surface/\(surfaceID)/destroy"
  }

  // MARK: - Repo.

  static func repoOpen(path: String) -> String {
    "cherrylily://repo/open?path=\(percentEncodeQueryValue(path))"
  }

  static func repoWorktreeNew(repoID: String, branch: String?, base: String?, fetch: Bool) -> String {
    var url = "cherrylily://repo/\(repoID)/worktree/new"
    var params: [String] = []
  ...
  static func settings(section: String?) -> String {
    guard let section else { return "cherrylily://settings" }
    return "cherrylily://settings/\(section)"
  }

  static func settingsRepo(repoID: String) -> String {
    "cherrylily://settings/repo/\(repoID)"
  }
