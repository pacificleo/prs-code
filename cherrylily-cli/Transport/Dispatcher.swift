...
/// Dispatches a deeplink URL to the running CherryLily app via its Unix domain socket.
...
nonisolated enum Dispatcher {
  ...
  static func resolveSocket() throws -> String {
    // Inside a CherryLily terminal — use the env var if the socket is still alive.
    ...
    if existing.count > 1 {
      throw SocketClient.Error.responseError(
        "Multiple CherryLily instances found. Run inside a CherryLily terminal or specify SUPACODE_SOCKET_PATH.\n"
    ...
    guard let path = try waitForSocket(timeout: 10.0) else {
      throw SocketClient.Error.responseError("Timed out waiting for CherryLily to start.")
    }
    return path
  }
  ...
  private static func launchApp() throws {
    ...
    process.arguments = ["-a", "CherryLily"]
    let stderrPipe = Pipe()
    ...
    guard process.terminationStatus == 0 else {
      ...
      let message = detail.isEmpty ? "Failed to launch CherryLily (exit \(process.terminationStatus))." : detail
  ...
  private static func waitForSocket(timeout: TimeInterval) throws -> String? {
    ...
    FileHandle.standardError.write(Data("Waiting for CherryLily to start...\n".utf8))
