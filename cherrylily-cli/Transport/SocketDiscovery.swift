...
/// Discovers CherryLily socket paths.
nonisolated enum SocketDiscovery {
  /// Returns the socket path from `$SUPACODE_SOCKET_PATH`.
  /// Only available inside a CherryLily terminal session.
  static func fromEnvironment() -> String? {
    EnvironmentDefaults.socketPath
  }

  /// Returns true if the socket path looks like a live CherryLily socket
