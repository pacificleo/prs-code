...
/// Sends a query to the running CherryLily app via socket, parses the response, and returns the data array.
nonisolated enum QueryDispatcher {
  static func query(resource: String, params: [String: String] = [:]) throws -> [[String: String]] {
    ...
    guard !response.isEmpty else {
      throw SocketClient.Error.responseError("Empty response from CherryLily.")
    }
    guard let parsed = try? JSONSerialization.jsonObject(with: response) as? [String: Any] else {
      throw SocketClient.Error.responseError("Malformed response from CherryLily.")
    }
